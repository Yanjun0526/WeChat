...
Robustness: the pipeline adds 9 five-topic-score models, 9 topic-PCA models, and 18 alternative-distance models.
...
How to read the output: the main complete-case n is 592. Role/gender coefficients describe group differences across agent-level outcome dimensions. The different outcome set appears here because Level 2 summarizes each agent's average diffusion pattern, not a single article-agent case.

Role/gender model signal: No coefficient in this block is below p<0.05; strongest observed term is `C(JobCategory)[T.1]` on `gender_assortativity_mean`: coef=0.030, p=0.664.
...
Supplementary network-position outcome signal: No coefficient in this block is below p<0.05; strongest observed term is `C(JobCategory)[T.1]` on `gender_assortativity_mean`: coef=0.030, p=0.664.
...
Key continuous matching signal: `z_MatchScore_mean` on `depth_mean`: coef=0.434, standardized beta=0.434, p=1.23e-10.

Key binary/proportion matching signal: `z_ProfessionContentMatch_mean` on `reshare_mean`: coef=0.023, standardized beta=0.023, p=6.85e-07.
...
Method: one row per agent. Primary DV: `log_agent_cascade_size_mean = log1p(cascade_size_mean)`. Controls: `JobCategory`, `agent_gender`, and `log_article_count_per_agent` in the core model. `article_count_per_agent` is calculated from `final_results.xlsx` and used as a stability/opportunity control. Main complete-case n: 592. Supplementary dependent variables include agent-level mean depth, reshare, structural virality, Wiener index, centrality, repeat exposure, and network composition.
